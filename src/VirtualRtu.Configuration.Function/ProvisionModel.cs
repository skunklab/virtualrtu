...
namespace VirtualRtu.Configuration.Function
{
    public class ProvisionModel
    {
        ...
        private async Task UpdateRtuMapAsync(ContainerEntity entity, string connectionString, string container,
            string filename)
        {
            ...
            foreach (var slave in entity.Slaves)
            {
                ...
                string requestUriString = UriGenerator.GetRtuPiSystem(entity.Hostname, entity.VirtualRtuId,
                    entity.DeviceId, slave.UnitId, true);
                string responseUriString = UriGenerator.GetRtuPiSystem(entity.Hostname, entity.VirtualRtuId,
                    entity.DeviceId, slave.UnitId, false);
        ...
        {
            JsonWebToken token =
                new JsonWebToken(config.SymmetricKey, claims, config.LifetimeMinutes, issuer, audience);
