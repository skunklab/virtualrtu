...
namespace VirtualRtu.Configuration.Vrtu
{
    ...
    public class RangeFilter : IModbusFilter
    {
        ...
        [JsonProperty("start")] public ushort StartAddress { get; set; }

        [JsonProperty("end")] public ushort EndAddress { get; set; }
        ...
        public bool Apply(ushort address, ushort qty, byte scope)
        {
            ...
            return !(max <= address && address >= StartAddress && address <= EndAddress && max >= StartAddress &&
                     max <= EndAddress);
        }
