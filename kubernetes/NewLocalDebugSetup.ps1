...
{	
    param ([string]$SubscriptionName, [string]$ResourceGroupName, [string]$Location, [string]$VirtualRtuId, [string]$PiraeusFile, [string]$StorageAcctName, [string]$IoTHubName, [string]$AppInsightsName)
    
    $subscriptionNameOrId = $SubscriptionName
    $filepath = $PiraeusFile
    ...
    $kind = "other"
    $rtuMapcontainer = "maps"
    $rtuMapFilename = "rtumap.json"
    $tableName = "gateway"
    ...
    $piconfig = Get-Content -Raw -Path $filepath | ConvertFrom-Json
    $apiTokens = $piconfig.apiSecurityCodes 

    #properties from Piraeus deployment
    $apiToken = $apiTokens.Split(";")[0]
    $hostname = $piconfig.dnsName + ".$Location.cloudapp.azure.com"
    $symmetricKey = $piconfig.symmetricKey
    ...
    Write-Host "Updated deploy.json file with virtualRtuId and App Insights instrumentationKey."
